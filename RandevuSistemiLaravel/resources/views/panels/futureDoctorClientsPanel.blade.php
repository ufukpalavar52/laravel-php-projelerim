...
for($i = 0; $i < 14; $i++)
{
    ...
    $clientList =  Client::showTodayDoctorClients($date,$_SESSION['user_id']); 
            ...
            <div class="panel-body" id="t<?php echo $i;?>" style="display: none;">
                <table class="table">
                    ...
                    <tr>
                        ...
                        <th colspan="3"><center>İşlemler</center></th>
                    </tr>
                    <?php
                    ...
                    <tr>
                        ...
                        <td><center>
                            ...
                            <button type="submit" <?= $disabled ?> title="Güncelle" class="btn btn-primary glyphicon glyphicon-pencil"></button>
                            {!! Form::close() !!}
                            </center>
                        </td>
                        <td><center>
                            {!! Form::open(['url'=>'musteri-gecmis-bilgileri']) !!}
                            <input type="number" type='text' class="hidden" name="id" value="{{ $list->id }}"/>
                            <button type="submit"  title="Geçmiş Bilgiler"  class="btn btn-success glyphicon glyphicon-calendar"></button>
                            {!! Form::close() !!}
                            </center>
                        </td>
                        <td><center>
                        <a href='#myDoctor<?= $list->id ?>' data-toggle="modal"  <?= $disabled ?> class="btn btn-danger glyphicon glyphicon-trash" title="Sil"></a>
